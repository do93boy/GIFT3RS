...
.chatRow .modBtns{opacity:.5;transition:opacity .15s;}
    ...
    <div style={{paddingBottom:40}} className="page">
      ...
      <div style={{padding:"20px"}}>
        ...
        {studioTab==="chat"&&<div style={{maxWidth:500}}>
          <div style={{fontSize:13,color:C.muted,marginBottom:12}}>{isLive?"Live chat with your viewers — hover a comment to pin, like or delete it":"Chat is available while live"}</div>
          {studioPinned&&<div style={{padding:"8px 12px",background:`${C.cyan}12`,border:`1px solid ${C.cyan}30`,borderRadius:10,display:"flex",alignItems:"center",gap:8,marginBottom:8}}><Ico n="star" s={13} c={C.cyan}/><div style={{flex:1,minWidth:0,fontSize:12}}><span style={{fontWeight:800,color:C.cyan}}>{studioPinned.u} </span><span>{studioPinned.t}</span></div><button onClick={()=>modStudio("unpin",studioPinned)} style={{background:"none",border:"none",cursor:"pointer",padding:0,display:"flex"}}><Ico n="close" s={13} c={C.muted}/></button></div>}
          <div className="settingsInner" style={{borderRadius:14,height:360,overflowY:"auto",padding:"12px",marginBottom:12,border:`1px solid ${C.border}`}}>
            ...
            {studioChat.map((m,i)=>(<div key={m.id||i} className="chatRow" style={{marginBottom:8,display:"flex",gap:8,alignItems:"flex-start"}}><div style={{width:24,height:24,borderRadius:"50%",background:m.type==="streamer"?C.amber:m.type==="system"?C.cyan:C.purple,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:800,color:"#000"}}>{(m.u||"?")[0]}</div><div style={{flex:1,minWidth:0}}><span style={{fontSize:12,fontWeight:800,color:m.type==="streamer"?C.amber:m.type==="system"?C.cyan:C.purple}}>{m.u} </span><span style={{fontSize:13,color:m.type==="system"?C.cyan:"inherit"}}>{m.t}</span>{m.likes>0&&<span style={{marginLeft:6,fontSize:11,color:"#FF6060",fontWeight:700}}>♥ {m.likes}</span>}</div>{m.type!=="system"&&m.type!=="gift"&&m.type!=="sub"&&<div className="modBtns" style={{display:"flex",gap:8,flexShrink:0,alignItems:"center"}}><button onClick={()=>modStudio("like",m)} title="Like" style={{background:"none",border:"none",cursor:"pointer",padding:0,display:"flex"}}><Ico n="heart" s={14} c="#FF6060"/></button><button onClick={()=>modStudio("pin",m)} title="Pin" style={{background:"none",border:"none",cursor:"pointer",padding:0,display:"flex"}}><Ico n="star" s={14} c={C.cyan}/></button><button onClick={()=>modStudio("delete",m)} title="Delete" style={{background:"none",border:"none",cursor:"pointer",padding:0,display:"flex"}}><Ico n="trash" s={14} c="#FF6060"/></button></div>}</div>))}
          </div>
